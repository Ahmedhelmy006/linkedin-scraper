...
class HumanLikeBehavior:
    ...
    def browse_feed(self, min_duration: Optional[float] = None, max_duration: Optional[float] = None) -> bool:
        ...
        try:
            ...
            feed_loaded = self.driver.wait_for_selector(".feed-shared-update-v2, .scaffold-layout", timeout=30000)
            
            if not feed_loaded:
                logger.warning("Feed elements not detected, proceeding anyway")
                time.sleep(5)
    ...
    def navigate_to_profile(self, profile_url: str) -> bool:
        ...
        try:
            ...
            # Wait for profile to load and verify we're on the right page
            ...
            if not profile_loaded:
                logger.warning("Profile elements not detected, proceeding anyway")
                time.sleep(5)
    ...
    def navigate_to_profile_section(self, profile_url: str, section: str) -> bool:
        ...
        try:
            ...
            if not section_loaded:
                logger.warning(f"{section.capitalize()} section elements not detected, proceeding anyway")
                time.sleep(5)
